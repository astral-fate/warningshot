...
\author{Fatimah Emad Eldin\\\small AI researcher, Trouv\'e Works\\\small\texttt{Fatimah@trouve.works}}
...
headline numbers from the committed results and prints agreement counts; 121
...
\textbf{All code, data and experiments reported in this paper are publicly
available} at \url{https://github.com/astral-fate/warningshot}, released under the MIT licence. The repository contains
the analysis code, the cached API responses every number is computed from, the
committed results, the figures, the full test suite, and the \LaTeX{} source of
this paper. Nothing required to reproduce a result is withheld, and no
credentials, paid tier or institutional access is needed to obtain any of it.
...
\texttt{python -m pytest} & 121 tests, no network \\
...
\paragraph{Code and data availability.} \textbf{All code, datasets and
experiments are publicly available} at \url{https://github.com/astral-fate/warningshot} under the MIT licence. The
underlying sources are themselves public and keyless: Wikimedia Pageviews
\cite{wikimedia2026pageviews}, the Hacker News Algolia API, and GDELT DOC 2.0
\cite{gdelt2026doc}. The repository ships the cached API responses alongside the
code, so every number recomputes offline at no cost; see
App.~\ref{sec:repro} for the commands. No restricted, proprietary or personal
data was used.
